...
bool SSH::isFrameworkUpToDate() {
  ...
  QFile mFrameworkVersionFile(QProcessEnvironment::systemEnvironment().value("WEBOTS_HOME")+"/projects/robots/robotis/libraries/robotis-op2/robotis/version.txt");
...
int SSH::updateFramework(const QString &username, const QString &password) {
  ...
  const QString robotisDir = webotsHomePath + "/projects/robots/robotis/libraries/robotis-op2/robotis";
...
int SSH::updateWrapper(const QString &username, const QString &password) {
  ...
  const QString webotsHomePath = QProcessEnvironment::systemEnvironment().value("WEBOTS_HOME");
  const QString managerDir = webotsHomePath + "/projects/robots/robotis/libraries/managers";
  const QString robotisDir = webotsHomePath + "/projects/robots/robotis";
  const QString controllerDir = webotsHomePath + "/projects/robots/robotis/controllers";
  ...
  // Hook demo program
  QString command = "echo "+password+" | sudo -S cp /" + mainDirectoryName() + "/Linux/project/webots/config/rc.local_custom /etc/rc.local";
...
bool SSH::isWrapperUpToDate() {
  ...
  QString version, versionInstalled;
  QStringList versionList, versionInstalledList;
  QFile mWrapperVersionFile(QProcessEnvironment::systemEnvironment().value("WEBOTS_HOME")+"/projects/robots/robotis/config/version.txt");
...
int SSH::startRemoteCompilation(const QString &IPAddress, const QString &username, const QString &password,bool makeDefaultController) {
  ...
  if (!mStdout.startsWith("Success")) {
    log("Robot is not in ready position.", true);
